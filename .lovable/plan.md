...
1. **Old price shown for increases too**
   Today the crossed-out original price and the badge only appear when you lower a price. After this change, any modified price shows the original one next to it, for all three cards and all four subscriptions:
   - Lower price: original crossed out + gold "−X%" badge (as today).
   - Higher price: original crossed out + a "+X%" badge in a distinct navy/gold styling so the two cases read differently.
...
- Sorting, subscription order, theme, and animations stay exactly as they are.
